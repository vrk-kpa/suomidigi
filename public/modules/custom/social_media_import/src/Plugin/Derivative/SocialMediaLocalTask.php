...
class SocialMediaLocalTask extends DeriverBase implements ContainerDeriverInterface {
  ...
  public function getDerivativeDefinitions($base_plugin_definition) {
    ...
    foreach (\Drupal::service('entity_type.bundle.info')->getBundleInfo($entity_type_id) as $bundle_id => $bundle_info) {
      ...
      $weight = 0;
      foreach (\Drupal::service('entity_display.repository')->getFormModes($entity_type_id) as $form_mode => $form_mode_info) {
      ...
      $weight = 0;
      foreach (\Drupal::service('entity_display.repository')->getViewModes($entity_type_id) as $view_mode => $form_mode_info) {
